...
class TASTER(BaselineModel):
    def __init__(self, args):
        super(TASTER, self).__init__()
        self.args = args
        self.dim = self.args.dim * 2
        self.sizes = args.sizes
        self.nentity = self.sizes[0]
        self.nrelation = self.sizes[1]
        self.n_day = self.sizes[3]
        self.rel_embeds_g = nn.Embedding(self.nrelation, self.dim)
        self.ent_embeds = nn.Embedding(self.nentity, self.dim)
        self.rel_embeds = nn.Embedding(self.nrelation, self.dim // 2)
        self.radio = torch.rand(self.nrelation)
        ...
        if self.args.shear:
            self.shear_embeds = nn.Embedding(self.n_day, self.args.dim * 2).cuda()
            ...
            self.trans_matrix = nn.Parameter(torch.Tensor(self.n_day, self.block_num * self.block_size * self.block_size))
    ...
    def forward(self, x):
        ...
        times = torch.arange(0, time_len, device=x.device, dtype=torch.long).repeat(batch_size).unsqueeze(1)
        x = x.repeat_interleave(time_len, 0)
        x = torch.reshape(x, (batch_size * time_len, -1))
        times = torch.reshape(times, (batch_size * time_len, 1))
        [heads, rels, tails] = [x[ : , i] for i in range(3)]
        # print(heads.shape, rels.shape, tails.shape, times.shape)
        score = self.forward_(heads.unsqueeze(1), rels.unsqueeze(1), tails.unsqueeze(1), times)
        score = torch.reshape(score, (batch_size, time_len))
        score = self.norm_event_score(score)
        return score
    
    def forward_(self, heads, rels, tails, date_ids, mask_part='tail'):
